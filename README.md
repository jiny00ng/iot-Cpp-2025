...
2. Class : 사용자 정의 자료형
    ...
    - 멤버 변수(객체의 속성) - private 영역
        - public 메소드를 통해 접근
            ...
            ```
    - 멤버함수(메서드 =  객체의 기능, 클래스 속 함수) - public 영역
...
C++ 프로그램은 실행 시 아래와 같은 메모리 영역으로 나뉘어 관리됩니다. 각각의 영역은 역할과 저장되는 데이터가 다릅니다.
...
| 메모리 영역 | 세부 내용 | 예시 |
|--------------|------------|------|
| 🟢 **Code Segment** | 프로그램 실행 코드 저장<br>함수 하나당 하나씩 배치됨 | `main()`, 사용자 정의 함수 |
| 🔴 **Data Segment** | 전역, static 변수 저장<br>문자열 리터럴도 여기에 포함됨 | `int a = 10;`, `"hello"` |
| 🔴 `.bss` | 초기화되지 않은 전역/정적 변수 | `static int x;` |
| 🔴 `.data` | 초기화된 전역/정적 변수 | `int y = 5;` |
| 🟡 **Heap** | 동적으로 사용자가 할당하는 공간 | `new`, `malloc` |
| 🔵 **Stack** | 함수 호출 시 생성되는 프레임:<br>- 지역변수<br>- 매개변수<br>- 리턴 주소 등 | `int x = 3;` (함수 내부) |
| 🟣 **Command Line / 환경변수** | `argc`, `argv`, 환경변수 등 실행 인자 | `argv[0]`, `PATH` |


---

#### ✅ 변수(저장할 공간)와 메모리 영역 대응

| 변수/데이터 종류 | 메모리 영역 |
|------------------|--------------|
| 지역변수           | Stack |
| 함수 매개변수       | Stack |
| 전역 변수 (초기화됨) | Data Segment (.data) |
| 전역 변수 (초기화 안됨) | Data Segment (.bss) |
| static 변수       | Data Segment |
| 문자열 리터럴      | Data Segment |
| 동적 변수 (`new`, `malloc`) | Heap |
| 함수 정의 코드     | Code Segment |

---

#### 🔎 참고

- **Stack**: 높은 주소에서 낮은 주소로 내려오며, 함수 호출 시 자동으로 메모리 할당 및 해제됨.
- **Heap**: 사용자가 직접 `new`, `delete` 또는 `malloc`, `free`로 관리해야 함.
- **Data Segment**: 정적 수명 변수 저장, 프로그램 실행 내내 유지됨.
- **Code Segment**: 읽기 전용, 함수당 하나씩 할당됨 (재사용 불가).


## 3일차 - STL(Standard Template Library)

### 📚 STL 주요 개념

- **STL**은 C++에서 제공하는 표준 템플릿 라이브러리로, 다양한 자료구조와 알고리즘을 효율적으로 제공함

---

### 📌 STL 자료구조 분류

#### 1. 시퀀스 컨테이너 (Sequential Containers)
...
- 데이터를 **키(Key)**를 기준으로 자동 정렬하여 저장
- 주로 **쌍(pair)** 형태로 데이터 구성
- 예시:
  - `set`, `multiset`
  - `map`, `multimap`
...
- C++ STL에서 제공하는 **중복 없는 집합** 자료구조
- 삽입한 원소는 **자동으로 정렬**됨 (기본: 오름차순)
- 내부적으로 **이진 탐색 트리 (Red-Black Tree)** 사용

#### ✅ 특징
- 중복된 값을 허용하지 않음
- 삽입, 삭제, 탐색의 평균 시간 복잡도: **O(log n)**